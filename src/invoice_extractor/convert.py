...
def convert_llm_responses_to_json():
    """
    Convert LLM responses stored in 'llm_responses' to JSON format and save them into 'invoice_json' directory.
    """
    llm_responses_dir = "llm_responses"
    invoice_json_dir = "invoice_json"
    
    # Create directory if it doesn't exist
    os.makedirs(invoice_json_dir, exist_ok=True)
    
    for response_file in os.listdir(llm_responses_dir):
        if response_file.endswith(".json"):
            with open(os.path.join(llm_responses_dir, response_file), "r") as f:
                data = json.load(f)
            
            # Save the JSON to the 'invoice_json' directory
            json_file_path = os.path.join(invoice_json_dir, response_file)
            with open(json_file_path, "w") as json_file:
